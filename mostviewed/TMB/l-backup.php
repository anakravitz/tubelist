...
function loadVideo($category = null)
{
    ...
    $response = file_get_contents("https://www.googleapis.com/youtube/v3/search?part=snippet&videoCategoryId=$category&safeSearch=none&publishedAfter=" . urlencode(getPublishAfterDate()) . "&publishedBefore=" . urlencode(getPublishBeforeDate()) . "&maxResults=50&order=viewCount&type=video&key=AIzaSyBRxS0JS_JD_FTarF784exuKNuCgCnJIy0");
...
function loadNonMusicVideo($categories = null)
{
    ...
    if (is_array($categories)) {
        foreach ($categories as $category) {
            $response = file_get_contents("https://www.googleapis.com/youtube/v3/search?part=snippet&videoCategoryId=$category&safeSearch=none&publishedBefore=2015-11-20T00%3A00%3A00Z&maxResults=48&order=viewCount&type=video&key=AIzaSyBRxS0JS_JD_FTarF784exuKNuCgCnJIy0");
    ...
    <div onClick="nextPage('<?php echo @$keyword; ?>', '<?php echo $nextPage; ?>')" class="next btn-primary">NEXT PAGE</div>
    <?php
}
